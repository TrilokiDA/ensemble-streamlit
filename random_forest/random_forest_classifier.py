# Created by trilo at 30-08-2024
...
df = pd.read_csv('toy_datasets/concertriccir2.csv')
...
max_features = st.sidebar.selectbox(
    'Max Features',
    ("sqrt", "log2", "max_features", "None")
)

if max_features == "max_features":
    max_features = int(st.sidebar.number_input('max_features'))

if max_features == "None":
    max_features = int(st.sidebar.number_input('max_features=n_features'))
...
if st.sidebar.button('Run Algorithm'):
    ...
    clf = RandomForestClassifier(n_estimators=n_estimators, random_state=42, bootstrap=bootstrap,
